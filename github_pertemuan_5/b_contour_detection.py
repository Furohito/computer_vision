...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
dir = "C:/Users/rende/OneDrive/Dokumen/data kuliah/semester 6/6 computer vision/pertemuan 5/tugas/"
nama_file_input = "nas_r"
sld = 0.01   #0.02 means that scanning lense diameter is 2% of image height (row).
var_th_normal = 0.1 #Any scanning region that has 0.2 of max_var will be detected.
sub_image_structure = [12, 12] #[8,8] means that the image is split into 8x8 sub-images.
warna_kontur = 0
print("")
...
# =============================================================================
# (vi) KODE TAMBAHAN: CROPPING GAMBAR BERKONTUR & LATAR PUTIH (TANPA CV2)
# =============================================================================
print("\n(vi) PROSES CROPPING GAMBAR BERKONTUR...")

# 1. Cari piksel kontur (selisih gambar grayscale asli dan buffer)
beda = np.abs(ori_gs.astype(int) - buffer.astype(int))
y_coords, x_coords = np.where(beda > 0)

# 2. Cek apakah ada kontur yang terdeteksi
if len(y_coords) > 0 and len(x_coords) > 0:
    ...
    y_max_crop = min(row, y_max + margin)
    x_min_crop = max(0, x_min - margin)
    x_max_crop = min(col, x_max + margin)

    # --- OPSI 1: GAMBAR ASLI DENGAN GARIS KONTUR ---
    pic_kontur = pic.copy()
    # Timpa piksel yang merupakan kontur dengan warna hitam [0, 0, 0]
    pic_kontur[y_coords, x_coords] = [0, 0, 0] 
    hasil_crop_opsi1 = pic_kontur[y_min_crop:y_max_crop, x_min_crop:x_max_crop]
    
    # --- OPSI 2: LATAR PUTIH BERSIH DENGAN GARIS KONTUR (SKETSA) ---
    # Bikin kanvas putih seukuran gambar asli
    kanvas_putih = np.ones_like(pic) * 255
    # Gambar kontur warna hitam di atas kanvas putih
    kanvas_putih[y_coords, x_coords] = [0, 0, 0]
    hasil_crop_opsi2 = kanvas_putih[y_min_crop:y_max_crop, x_min_crop:x_max_crop]

    # 3. Simpan kedua gambar menggunakan matplotlib
    plt.imsave(dir + f"Crop_Asli_Kontur_{nama_file_input}.jpg", hasil_crop_opsi1)
    plt.imsave(dir + f"Crop_Putih_Kontur_{nama_file_input}.jpg", hasil_crop_opsi2)
    
    print("     => Berhasil menyimpan Opsi 1 (Gambar Asli+Kontur) dan Opsi 2 (Sketsa Putih+Kontur)!")
    
    # (Opsional) Tampilkan Opsi 2 di layar biar langsung keliatan
    plt.figure("Hasil Crop Kontur & Latar Putih")
    plt.subplot(1, 2, 1)
    plt.imshow(hasil_crop_opsi1)
    plt.title("Opsi 1: Asli + Kontur")
    plt.axis('off')
    
    plt.subplot(1, 2, 2)
    plt.imshow(hasil_crop_opsi2)
    plt.title("Opsi 2: Latar Putih + Kontur")
    plt.axis('off')
    
    plt.show()

else:
    print("     Gagal crop: Tidak ada kontur yang terdeteksi.")
